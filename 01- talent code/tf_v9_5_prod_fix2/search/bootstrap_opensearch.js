...
const defs = [
  ['tf_posts','mappings_posts.json'], ['tf_people','mappings_people.json'], ['tf_jobs','mappings_jobs.json'],
  ['tf_courses','mappings_courses.json'], ['tf_interactions','mappings_interactions.json'],
  ['tf_applicants','mappings_applicants.json'], ['tf_classes','mappings_classes.json'],
  ['tf_enrollments','mappings_enrollments.json'], ['tf_academicSessions','mappings_academicSessions.json']
]
...
async function main(){ for (const [base, file] of defs){ const body = JSON.parse(fs.readFileSync('search/'+file,'utf8')); await createOrKeep(base, body) } await applyILM(); console.log('bootstrap done.') }
