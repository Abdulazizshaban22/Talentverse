...
function lrScore(hit, pr){ const bm25 = hit._score || 0; const w = model.weights || { text_bm25:1.0, graph_pr:1.0 }; return (w.text_bm25*bm25) + (w.graph_pr*(pr||0)) }
router.get('/fusion', async (req,res)=>{
  ...
  const session = driver.session(); const prMap = new Map()
  try { const neo = await session.run('MATCH (u:Person {id:$id})-[:FOLLOWS*0..1]->(x) RETURN x.id AS id, 0.1 AS score LIMIT 100', { id: viewerId }); neo.records.forEach(rec=> prMap.set(rec.get('id'), rec.get('score'))) } catch {}
  await session.close()
  const fused = hits.map(h=>({ id: h._source.id, _source:h._source, score: lrScore(h, prMap.get(h._source.authorId)||0) })).sort((a,b)=>b.score-a.score)
