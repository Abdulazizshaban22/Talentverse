...
router.get('/match', async (req,res)=>{
  ...
  try{
    ...
    const jsrc = job.body?._source || job?._source || {}
    const skills = jsrc.skills || []
    const r = await client.search({ index: 'tf_people@prod', size: 20, query: skills.length? { terms: { "skills.keyword": skills } } : { match_all:{} } })
    res.json({ ok:true, jobId, candidates: r.hits.hits.map(h=>h._source) })
  }catch(e){ res.status(404).json({ ok:false, error:'JOB_NOT_FOUND' }) }
})
