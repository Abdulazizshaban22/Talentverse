...
router.post('/oneroster/upload', upload.single('file'), async (req,res)=>{
  const file = req.file?.path; if (!file) return res.status(400).json({ ok:false, error:'NO_FILE' })
  const text = fs.readFileSync(file, 'utf8'); const rows = []
  await new Promise((resolve,reject)=>{ parse(text, { columns:true, trim:true }, (err, recs)=>{ if (err) reject(err); else { rows.push(...recs); resolve() } }) })
  ...
  if (header.includes('startDate') && header.includes('endDate')) index = 'tf_academicSessions@prod'
  await bulkIndex(index, rows); res.json({ ok:true, index, imported: rows.length })
})
