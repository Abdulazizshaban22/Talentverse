...
resource "aws_lb_target_group" "api" {
  name="${var.project}-api-tg" port=4000 protocol="HTTP" vpc_id=var.vpc_id target_type="ip"
  health_check { path="/health" }
}
resource "aws_lb_target_group" "web" {
  name="${var.project}-web-tg" port=3000 protocol="HTTP" vpc_id=var.vpc_id target_type="ip"
  health_check { path="/" }
}
resource "aws_lb_listener" "http" {
  load_balancer_arn=aws_lb.app_alb.arn port=80 protocol="HTTP"
...
resource "aws_lb_listener_rule" "api_rule" {
  ...
  condition { path_pattern { values=["/ai/*","/privacy/*","/social/*","/scholarships/*","/k12/*","/school/*","/gov/*","/trust/*","/payments/*","/compliance/*","/profile/*","/health"] } }
}
