...
PAYLOAD=$(jq -n --arg m "$(cat "$MODEL_FILE")" --arg fs "$FEATURESET" \
  '{model:{name:"'"$MODEL_NAME"'", model:{type:"model/xgboost+json", definition:$m}, feature_set:$fs}}')

curl $CURL_OPTS $AUTH_OPT -XPOST "$OPENSEARCH_NODE/_ltr/_model/$MODEL_NAME" \
