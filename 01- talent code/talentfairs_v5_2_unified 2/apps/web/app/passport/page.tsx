...
export default function TalentPassport(){
  ...
  useEffect(()=>{ (async()=>{
    try{ const r = await fetch('http://localhost:4000/profile/demo'); const dj = await r.json(); setProfile(dj.profile); setQr(genQR(`TF-Passport:${dj.profile?.id||'guest'}`)) }catch{ setProfile({id:'guest', name:'Guest User'}) }
  })() }, [])
