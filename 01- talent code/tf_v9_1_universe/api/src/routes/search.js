...
const model = fs.existsSync('ai/model.json') ? JSON.parse(fs.readFileSync('ai/model.json','utf8')) : { weights:{ text_bm25:1.0, graph_pr:1.0 } }
...
router.get('/fusion', async (req,res)=>{
  ...
  // 2) PageRank شخصي (تبسيط: قيمة ثابتة أو من Neo4j إذا توفر عقد)
