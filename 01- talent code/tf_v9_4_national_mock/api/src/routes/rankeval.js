...
router.post('/ndcg', (req,res)=>{
  ...
  // persist mock prod metric
  try{
    const old = fs.existsSync(metricsFile) ? JSON.parse(fs.readFileSync(metricsFile,'utf8')) : []
    old.push({ ts: Date.now(), ndcg: v, variant: 'prod' })
    fs.writeFileSync(metricsFile, JSON.stringify(old, null, 2))
  }catch{}
  res.json({ ok:true, ndcg: v })
})

router.get('/metrics', (_req,res)=>{
  const data = (fs.existsSync(metricsFile) ? JSON.parse(fs.readFileSync(metricsFile,'utf8')) : [])
