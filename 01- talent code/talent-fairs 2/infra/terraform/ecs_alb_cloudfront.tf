module "alb" {
  ...
  https_listeners = [] # you may switch to HTTPS on ALB if you terminate TLS there
  target_groups = [
...
module "ecs_services" {
  ...
  container_definitions = [
    {
      name  = "api"
      image = "public.ecr.aws/docker/library/node:20-alpine" # replace with your ECR image
      port_mappings = [{ containerPort = 4000 }]
    },
    {
      name  = "ai-core"
    ...
    },
    {
      ...
      port_mappings = [{ containerPort = 4100 }]
    }
  ]
}
