...
const authFailures = new client.Counter({ name:'tf_acl_auth_fail_total', help:'ACL/UMA auth failures', registers:[register] })
...
io.use(async (socket, next)=>{
  try{
    ...
    // @ts-ignore
    socket.data.user = { sub: payload.sub, roles: extractRoles(payload), authorization: payload?.authorization }
    return next()
  }catch(e){
    authFailures.inc()
...
io.on('connection', (socket)=>{
  socket.on('join', async ({ channel })=>{
    ...
    if(!user) return
    const allowed = (await getAllowedRoles(channel)).map(s=>String(s).toLowerCase())
    const hasRole = (user.roles||[]).some((r:string)=> allowed.includes(r))
    const hasPolicy = hasUmaPermission(user, `channel:${channel}`, 'join')
  ...
  socket.on('typing', async ({ channel })=>{
    ...
    const allowed = (await getAllowedRoles(channel)).map(s=>String(s).toLowerCase())
    const hasRole = (user.roles||[]).some((r:string)=> allowed.includes(r))
    const hasPolicy = hasUmaPermission(user, `channel:${channel}`, 'type')
    if(!(hasRole || hasPolicy)) return
...
// ---- Payments: Tap Google Pay + mada (redirect) ----
app.post('/payments/tap/charge', async (req,res)=>{
  const { amount=10, currency='SAR', customerId='cust_1', googlePay=false } = req.body||{}
  ...
  const body:any = {
    ...
    source: googlePay ? { id: 'src_google_pay' } : { id: 'src_all' },
    redirect: { url: ret }
  }
  ...
  await prisma.auditLog.create({ data:{ actorId:'system', action:'tap.charge', data: dj } })
  res.json(dj)
})

app.get('/payments/tap/return', async (req,res)=>{
  const id = String(req.query.tap_id||req.query.charge_id||'')
  await prisma.auditLog.create({ data:{ actorId:'system', action:'tap.return', data: { id } } })
  res.json({ ok:true, id })
})

// ---- Payments: HyperPay webhook + status ----
...
app.get('/health', (_req,res)=> res.json({ ok:true, v:'2.9' }))

server.listen(port, ()=> console.log('API v2.9 listening on', port))
